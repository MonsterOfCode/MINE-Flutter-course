...
class AnimatedInput extends StatelessWidget {
  ...
  final Animation<double> containerOpactiyAnim;
  ...
  AnimatedInput({this.controller})
      : containerOpactiyAnim = Tween(begin: 0.0, end: 1.0).animate(
          CurvedAnimation(
            parent: controller,
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(animation: controller, builder: _animated);
  }
  ...
  Widget _animated(context, child) {
    return Container(
      ...
      child: Opacity(
        opacity: containerOpactiyAnim.value,
        child: Row(
          children: [
            SizedBox(width: 20),
            Icon(Icons.search),
